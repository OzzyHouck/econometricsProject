...
allNominees <- read.csv("allNominees.csv", header = T) %>% 
  rename(movie = Film) %>% 
  select(movie, Awards)
otherAwards <- allNominees %>% 
  filter(allNominees$movie %in% nominees) 
...
modelWinner <- lm(percent_change ~ days + as.numeric(viewership) + friSat + Awards + after, data = dfWinner)
modelLoser <- lm(percent_change ~ days + as.numeric(viewership) + friSat + Awards + after, data = dfLoser)
