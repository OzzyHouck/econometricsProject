...
```{r data fetching and cleaning, echo = F}
...
```{r Summary Statistics and visualizations, echo = F}
awardSeason %>% 
...
```{r  initial models, results = 'asis', echo = F}
...
```{r checking for autocorrelation, echo = F}
ACtest <- df %>%
  group_by(movie) %>%
...
```{r testing and correcting Hskedat, results = 'asis' , echo = F}
#testing for H
...
```{r cut stuff I didnt feel comfortable deleting, echo = F}
# df <- df %>% 
