...
otherAwards <- allNominees %>% 
  filter(allNominees$Film %in% nominees & !allNominees$Film %in% winners) 
...
df %>% 
  ...
  ggplot(aes(x = date, y = percent_change, color = as.factor(friSat))) +
  geom_point() 
...
<<<<<<< HEAD
model <- lm(diffMeanChange ~ days + winner + as.numeric(viewership) + otherAwards$Awards, data = df)
modelRestricted <- lm(diffMeanChange ~ days + winner, data = df)
modelRestricted2 <- lm(diffMeanChange ~  winner + as.numeric(viewership), data = df)
summary(model)
=======
...
ggplot(df, aes(x = days, y = residuals, color = as.factor(winner))) +
  geom_point()

>>>>>>> 13311a8c225b8f1107c2e0f9db342a442f8463e2

stargazer(model, title = "Initial Model", digits = 5, covariate.labels = c("Days in Theater", "Best Picture Winner", "Oscar Viewership"), dep.var.labels = c("Difference in Mean % BOR Change"), header = FALSE, type = "latex")
```

$$\hat{\theta} = \frac{\frac{0.2033 - .198}{1}}{\frac{1 - .2033}{57 - 4}} \sim F(1, 53)$$
```{r}
var.test(model, modelRestricted2, alternative = "two.sided")
```
...
```
$$PRF: \bar{\Delta}\%BOR^{before} - \bar{\Delta}\%BOR^{after} = Winner + Days$$
>>>>>>> ff320c61d6abef95009b66c8473f1153c29d9fd4
