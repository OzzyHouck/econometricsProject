...
dfLoser <- df %>% 
  filter(winner == 0)
model <- lm(percent_change ~ days + as.numeric(viewership) + friSat + Awards + winner, data = df)
modelRestricted <- lm(percent_change ~ days + friSat + winner, data = df) #+ as.numeric(viewership) + Awards
