...
<<<<<<< HEAD
awardSeason <- boxoffice(dates = awardDates) #data pull
=======
awardSeason <- boxoffice(dates = awardDates)
awardSeasonNominees <- awardSeason %>%
      filter(awardSeason$movie %in% bestPicture$entity)
>>>>>>> 2ab93949d2e66dc55da8fc88b133221d61712ef3
...
nomineesList <- bestPicture$entity
...
```{r}
ggplot(df, aes(x = date, y = diffMeanChange)) +
  geom_point()
```
